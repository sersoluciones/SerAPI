 ...
 */
export class GraphQLCrudEdit extends BaseCrudEdit {
    ...
    @ViewChild(CrudEditToolsComponent) toolbarEdit: CrudEditToolsComponent;
    ...
    processListData(response: any): void {
        response.data[this.graphQuery.list.name].forEach((element: any) => {
            const item = this.listObject(element);
            item.addControl('_isSelected', new FormControl(false));
